...
extension HomeHiPartViewController: UICollectionViewDelegate {
	func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
		let masterViewController = self.parent!.parent!.parent as! MasterViewController
		...
		if hipartViewControler.tabLayout == nil{
			hipartViewControler.reservedTabIndex = indexPath.row + 1
		}else{
			hipartViewControler.tabLayout.selectedTab = indexPath.row + 1
		}
	}
}
