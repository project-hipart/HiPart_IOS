...
class PortfolioUploadViewController: UIViewController {
	...
	@objc private func willShow(notification: NSNotification){
		...
		let frame = self.urlTextField.frame
...
extension PortfolioUploadViewController : UITextFieldDelegate{
...
extension NSNotification{
	var height : CGFloat{
		return (self.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.height ?? 0
	}
	var duration : Double{
		return self.userInfo?[UIResponder.keyboardAnimationDurationUserInfoKey] as? Double ?? 0
	}
}
//MARK: Actions
extension PortfolioUploadViewController{
	...
	@IBAction func tapUploadButton(_ sender: Any) {
		...
		switch UserRepository.shared.myType!{
		case .Creator:
		...
		case .PD:
			PortfolioRepository.shared.editorUpload(thumbnail: thumbnail!, thumbnailUrl: thumbnailUrl.absoluteString!, url: url, title: title, content: content){json in
		...
		case .Etc:
			PortfolioRepository.shared.etcUpload(thumbnail: thumbnail!, thumbnailUrl: thumbnailUrl.absoluteString!, url: url, title: title, content: content){json in
